...
export function mapMedusaProductToApiProduct(product: MedusaProduct): ApiProduct {
  ...
  return {
    ...
    inStock: product.variants?.some((v) => !v.manage_inventory || v.inventory_quantity === undefined || v.inventory_quantity > 0) ?? true,
    inventory: product.variants?.reduce((sum, v) => sum + (v.inventory_quantity || 100), 0) || 0,
    avgRating: 0,
    reviewCount: 0,
    variants: product.variants?.map((variant) => ({
      ...
      priceAdjustment: 0,
      inStock: !variant.manage_inventory || variant.inventory_quantity === undefined || variant.inventory_quantity > 0,
